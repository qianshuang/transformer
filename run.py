...
def evaluate_ml(sess, x_, y_):
    """评估在某一数据上的准确率和损失"""
    data_len = len(x_)
    batch_eval = batch_iter_ml(x_, y_, config.batch_size)
    total_loss = 0.0
    total_acc = 0.0
...
def train():
    ...
    for epoch in range(config.num_epochs):
        ...
        batch_train = batch_iter_ml(x_train, y_train, config.batch_size)
        ...
        for x_batch, y_batch in batch_train:
            feed_dict = {
                ...
                model.att_keep_prob: config.att_keep_prob,
                # model.seq_length: len_batch
                # model.input_task: len_batch
            }
            ...
            if total_batch % config.print_per_batch == 0:
                ...
                feed_dict[model.att_keep_prob] = 1.0
                loss_train, acc_train = session.run([model.loss, model.acc], feed_dict=feed_dict)
                # loss_val, acc_val = evaluate(session, x_val, y_val, len_val)
                loss_val, acc_val = evaluate_ml(session, x_val, y_val)
...
def test():
    ...
    loss_test, acc_test = evaluate_ml(session, x_test, y_test)
...
if __name__ == '__main__':
    ...
    config.embeddedPosition = fixedPositionEmbedding(config.batch_size, config.seq_length)
